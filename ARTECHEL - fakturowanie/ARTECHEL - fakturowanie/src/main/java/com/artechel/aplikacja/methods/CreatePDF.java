...
public class CreatePDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter pdfWriter, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws Exception {
        ...
        List<Invoice_product> productsList = (List<Invoice_product>) model.get("listProducts");
        Customer customer = (Customer) model.get("customer");
        Payer payer = (Payer) model.get("payer");

        addMetaData(document, invoice);
        addContent(document, invoice, productsList, customer, payer);
    }

    private void addMetaData(Document document, Invoice invoice) {
        document.addTitle("Faktura nr " + invoice.getInvoice_number());
        document.addAuthor("Artykuly Techniczne Motoryzacyjne i Elektryczne Dorota Świtała");
    }

    private void addContent(Document document, Invoice invoice, List<Invoice_product> productsList, Customer customer, Payer payer) throws DocumentException, ParseException {
        ...
        createInvoiceDataTable(getFonts(), pdfObject, invoice);
        addEmptyLine(pdfObject, 1, getFonts());
        ...
        addEmptyLine(pdfObject, 1, getFonts());
        createPaymentTable(getFonts(), pdfObject, invoice);
        addEmptyLine(pdfObject, 1, getFonts());
        createProductsTable(getFonts(), pdfObject, productsList);
        addEmptyLine(pdfObject, 1, getFonts());
        createSumTable(getFonts(), pdfObject, productsList);
        addEmptyLine(pdfObject, 1, getFonts());
        createValueTable(getFonts(), pdfObject, invoice);
    ...
    private void createProductsTable(Hashtable<String, Font> fontList, Paragraph pdfObject, List<Invoice_product> productsList) {
        ...
        for (int i = 0; i < productsList.size(); i++) {
            ...
            PdfPCell product = new PdfPCell(new Phrase(productsList.get(i).getProduct_name(), fontList.get("font10")));
            ...
            PdfPCell jm = new PdfPCell(new Phrase(productsList.get(i).getUnit(), fontList.get("font10")));
            ...
            String quantityFixed = df.format(productsList.get(i).getQuantity());
            ...
            String nettoPriceFixed = df.format(productsList.get(i).getPrice_netto());
            ...
            PdfPCell vat = new PdfPCell(new Phrase(String.valueOf(productsList.get(i).getVat()), fontList.get("font10")));
            vat.setHorizontalAlignment(Element.ALIGN_CENTER);
            vat.setVerticalAlignment(Element.ALIGN_MIDDLE);
            tableProducts.addCell(vat);

            String bruttoPriceFixed = df.format(productsList.get(i).getPrice_brutto());
            ...
            String nettoValueFixed = df.format(productsList.get(i).getValue_netto());
            ...
            String bruttoValueFixed = df.format(productsList.get(i).getValue_brutto());
    ...
    private void createSumTable(Hashtable<String, Font> fontList, Paragraph pdfObject, List<Invoice_product> productsList) {
        ...
        for (int i = 0; i <= productsList.size() - 1; i++) {
            if (productsList.get(i).getVat() == 0) {
                netto0 = netto0 + productsList.get(i).getValue_netto();
                brutto0 = brutto0 + productsList.get(i).getValue_brutto();
                vat0 = vat0 + (brutto0 - netto0);
            } else if (productsList.get(i).getVat() == 8) {
                netto8 += productsList.get(i).getValue_netto();
                brutto8 += productsList.get(i).getValue_brutto();
                vat8 += (brutto8 - netto8);
            } else if (productsList.get(i).getVat() == 23) {
                netto23 += productsList.get(i).getValue_netto();
                brutto23 += productsList.get(i).getValue_brutto();
    ...
    private void createValueTable(Hashtable<String, Font> fontList, Paragraph pdfObject, Invoice invoice) {
        ...
        DecimalFormat df = new DecimalFormat("0.00");
        String invoiceValue = df.format(invoice.getValue_brutto());
        ...
        PdfPCell labelValue = new PdfPCell(new Phrase("Należność ogółem:", fontList.get("font10")));
    ...
    private void createInvoiceDataTable(Hashtable<String, Font> fontList, Paragraph pdfObject, Invoice invoiceObj) throws ParseException {
        ...
        PdfPCell invoice = new PdfPCell(new Phrase("FAKTURA VAT nr", fontList.get("fontBold11")));
